...
class VPTree:
    ...
    def _select_vp(self, start, end):
        return randrange(start, end)

    def _select_vp_sampling(self, start, end):
        n = end - start
        s = int(min(100, n) / 10)
        mid = s // 2
        arr_sample_ids = sample(list(range(start, end)), s)
        arr_eval_ids = sample(list(range(start, end)), s)
        arr_eval = [self.__dataset[i] for i in arr_eval_ids]
        best_spread = 0.0
        best_i = start
        for p_i in arr_sample_ids:
            _, p = self.__dataset[p_i]
            for i, (_, q) in enumerate(arr_eval):
                arr_eval[i] = self.__distance(p, q), q
            quickselect_tuple(arr_eval, 0, s, mid)
            mu, _ = arr_eval[mid]
            spread = 0.0
            for d, _ in arr_eval:
                spread += (d - mu)**2
    ...
    def _build_rec(self, start, end):
        ...
        mid = (end + start) // 2
        i = self._select_vp_sampling(start, end)
        #i = start
        self.__dataset[i], self.__dataset[start] = self.__dataset[start], self.__dataset[i]
        _, v_point = self.__dataset[start]
        for j in range(start + 1, end):
            _, p = self.__dataset[j]
            self.__dataset[j] = self.__distance(v_point, p), p
        ...
        if v_radius <= self.__leaf_radius:
            ...
            left = self._build_rec(start, mid)
